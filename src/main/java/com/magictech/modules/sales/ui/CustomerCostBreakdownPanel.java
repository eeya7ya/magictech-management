...
/**
 * Reusable Cost Breakdown UI component for customer orders with purple theme
 * Auto-calculates total based on formula:
 * Total = items_total + (items_total * tax_rate) - (items_total * sale_offer_rate) +
 *         installation_cost + licenses_cost + additional_cost
 */
public class CustomerCostBreakdownPanel extends VBox {
    ...
    // UI Components
    private Label itemsSubtotalValue;
    ...
    private CustomerCostBreakdown breakdown;

    // Callback for save action
    private Consumer<CustomerCostBreakdown> onSaveCallback;

    public CustomerCostBreakdownPanel() {
        buildUI();
        setupListeners();
    }
    ...
    private void buildUI() {
        ...
        // Title
        Label title = new Label("💰 Cost Breakdown");
        ...
        // Items Subtotal (read-only, auto-calculated)
        grid.add(createLabel("Items Subtotal:"), 0, row);
        itemsSubtotalValue = createValueLabel("$0.00");
        grid.add(itemsSubtotalValue, 1, row++);
    ...
    /**
     * Set items subtotal (from order items)
     */
    public void setItemsSubtotal(BigDecimal subtotal) {
        this.itemsSubtotal = subtotal;
        itemsSubtotalValue.setText("$" + subtotal.setScale(2, RoundingMode.HALF_UP).toString());
        recalculate();
    }
     ...
     */
    public void loadBreakdown(CustomerCostBreakdown breakdown) {
        this.breakdown = breakdown;

        if (breakdown != null) {
            setItemsSubtotal(breakdown.getItemsSubtotal());
    ...
    private void recalculate() {
        try {
            ...
            BigDecimal taxAmount = itemsSubtotal.multiply(taxRate);
            ...
            BigDecimal discountAmount = itemsSubtotal.multiply(offerRate);
            discountAmountValue.setText("$" + discountAmount.setScale(2, RoundingMode.HALF_UP).toString());

            // Calculate total
            BigDecimal total = itemsSubtotal
                    .add(taxAmount)
    ...
    private void handleSave() {
        if (breakdown == null) {
            breakdown = new CustomerCostBreakdown();
        }

        // Update breakdown object
        breakdown.setItemsSubtotal(itemsSubtotal);
     ...
     */
    public void setOnSave(Consumer<CustomerCostBreakdown> callback) {
     ...
     */
    public CustomerCostBreakdown getBreakdown() {
