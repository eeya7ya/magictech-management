...
/**
 * Pricing Storage Controller
 * Shows: ID, Manufacture, Product Name, Code, Serial Number, Availability, Price
 */
@Component
public class PricingStorageController extends BaseStorageModuleController {

    @Override
    protected ModuleStorageConfig getModuleConfig() {
        return ModuleStorageConfig.PRICING;
    }
        ...
        result.ifPresent(vm -> {
            ...
            saveTask.setOnSucceeded(e -> {
                ...
                Platform.runLater(() -> {
                    ...
                    updateEmptyState();
                    showLoading(false);
                    showSuccess("✓ Pricing item added!");
    ...
    protected void handleBulkDelete() {
        ...
        confirm.setHeaderText("Delete " + selected.size() + " pricing item(s)?");
        confirm.setContentText("This will remove the pricing information for these items.");
    ...
    /**
     * Create dialog - INCLUDES PRICE FIELD for Pricing
     */
    private Dialog<StorageItemViewModel> createItemDialog(StorageItemViewModel existing) {
        ...
        dialog.setTitle(existing == null ? "Add Pricing Item" : "Edit Pricing Item");
        ...
        qtySpinner.valueProperty().addListener((obs, old, newVal) -> {
            if (newVal > 0) {
                availLabel.setText("Status: ✅ Available for Pricing");
                availLabel.setStyle("-fx-font-weight: bold; -fx-text-fill: #eab308;");
            } else {
